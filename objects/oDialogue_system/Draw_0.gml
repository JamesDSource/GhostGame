...
for(var i = 1; i < iteration; i++)
{
	...
	{
		if(string_char_at(message, i) == " ") modifier = TEXTMODIFIERS.None;
		else if(!spoiled) spoiled = true; 
		var offset_x;
		...
		if(scale >= 1)
		{
			...
			for(var ii = 0; ii < scale; ii++) draw_text_transformed(draw_x + offset_x + mod_x -(ii + 1), draw_y + y_offset + mod_y + verticle_shift/2 -(ii + 1), string_char_at(message, i), scale, scale, 0);
		}
		draw_set_color(go_back_color); 
		draw_text_transformed(draw_x + offset_x + mod_x, draw_y + y_offset + mod_y + verticle_shift/2, string_char_at(message, i), scale, scale, 0);
